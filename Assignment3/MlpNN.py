...
class MlpNN():
    ...
    def __init__(self, learning_rate, momentum, weight_decay, nclasses, vectorsize, hiddenlayersize):
        ...
        self.W1 = tf.Variable(tf.random_normal([vectorsize, hiddenlayersize], stddev=0.1), name="weights")
        self.b1 = tf.Variable(tf.zeros([hiddenlayersize]), name="biases")
        self.l1 = tf.tanh(tf.matmul(self.x, self.W1) + self.b1)

        self.W2 = tf.Variable(tf.random_normal([hiddenlayersize, nclasses], stddev=0.1), name="weights")
        self.b2 = tf.Variable(tf.zeros([nclasses]), name="biases")
        self.y = tf.matmul(self.l1, self.W2) + self.b2
        ...
        self.loss_function = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.y, self.y_) + weight_decay * (tf.nn.l2_loss(self.W1)+tf.nn.l2_loss(self.W2)))
